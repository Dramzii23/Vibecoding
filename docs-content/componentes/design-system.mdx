---
title: Design system
description: Colores, tipografía, espaciado y reglas de UI/UX de Vibecoding.
order: 3
---

<StackBadge tech={["tailwind", "daisyui"]} />

Vibecoding trae un **design system** listo, con estilo **Flat Design** (limpio, accesible, rápido). Todo se apoya en tokens semánticos de daisyUI, así que cambiar una variable actualiza toda la app. Esta página es tu referencia; la fuente de verdad completa vive en `design-system/vibecoding/MASTER.md`.
...
El tema `vibecoding` (claro) es el default. Hay un `vibecoding-dark` listo en `web/app/globals.css`: si activas `data-theme="vibecoding-dark"` en el `<html>`, toda la app cambia con contraste correcto, porque todo usa tokens semánticos.
